...
subjects_per_study = (
    emb_good.obs.groupby("study").agg({"donor_id": "unique"}).loc[studies_to_keep, :]
)
...
mask = (
    ...
    & (
        emb_good.obs.ann_level_2.isin(ct_to_keep)
        | emb_good.obs.ann_level_2_pred.isin(ct_to_keep)
    )
)
...
sc.tl.umap(adata)
adata.write(
    "/lustre/groups/ml01/code/lisa.sikkema/mapqc/tests/data/input/mapped_q_and_r.h5ad"
)
```
